...
// guard base su cookie di sessione
router.beforeEach((to, from, next) => {
  const loggedIn = document.cookie.includes('session=') // o altro nome cookie
  if (to.path.startsWith('/app') && !loggedIn) return next({ name: 'login' })
  if (to.name === 'login' && loggedIn)       return next({ name: 'dashboard' })
  next()
})
